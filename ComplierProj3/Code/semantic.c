...
TableItem VarDec(Tree root, Type type, int flag){ // insert an item(varity defination)
    ...
    if (!strcmp(root->child->name, "ID")){
        ...
        Type arrayType;
        Type toBeGen;
        arrayType = newType(ARRAY, 2, NULL, temp->brother->brother->i);
        toBeGen = arrayType;
        while(!strcmp(temp->child->name, "VarDec")){
            toBeGen->u.array.eleType = newType(ARRAY, 2, NULL, temp->child->brother->brother->i);
            toBeGen = toBeGen->u.array.eleType;
            temp = temp->child;
        }
        ...
        //     temp = temp->child;
        // }
        char *name = temp->child->id;
        toBeGen->u.array.eleType = copyType(type);
